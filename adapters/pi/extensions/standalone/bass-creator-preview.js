...
const BassCreatorPreviewPlugin = async () => ({ tool: { bass_creator_preview: (0, plugin_1.tool)({ description: "Create deterministic local-only BASS previews without network or remote operations.", args: { projectName: plugin_1.tool.schema.string().optional(), artifactType: plugin_1.tool.schema.string(), title: plugin_1.tool.schema.string(), evidence: plugin_1.tool.schema.array(plugin_1.tool.schema.object({ type: plugin_1.tool.schema.string(), source: plugin_1.tool.schema.string(), location: plugin_1.tool.schema.string(), classification: plugin_1.tool.schema.string(), confidence: plugin_1.tool.schema.string(), claim: plugin_1.tool.schema.string().optional(), relatedItemId: plugin_1.tool.schema.string().optional() })), assumptions: plugin_1.tool.schema.array(plugin_1.tool.schema.string()).optional(), targetId: plugin_1.tool.schema.string().optional(), promoteTo: plugin_1.tool.schema.string().optional() }, async execute(args, context) {
                ...
                if (badProject(name) || !dir(projects))
                    return JSON.stringify(blocked("Selected project is invalid or unavailable."));
                const project = (0, node_path_1.join)(projects, name);
                if (!dir(project) || (0, node_fs_1.lstatSync)(project).isSymbolicLink())
                    return JSON.stringify(blocked("Selected project containment cannot be verified."));
                ...
                if (!types.has(type) || !title || (!evidence.length && !assumptions.length) || evidence.some((e) => !e || !evidenceTypes.has(e.type) || !e.source || !e.location || !classes.has(e.classification) || !e.confidence) || assumptions.some((assumption) => !assumption))
                    return JSON.stringify(blocked("Input requires a supported artifact type, title, and typed complete cited evidence or explicit assumptions."));
                ...
                if (!evidence.some((e) => e.classification === "Fact" || e.classification === "Inference"))
                    return JSON.stringify(blocked("Evidence requires at least one Fact or Inference.", render(newId, type, title, evidence, explicitAssumptions, related, artifactFile, String(args.targetId || "")), ["Evidence has no Fact or Inference."], ungroundedQuestions, ungroundedConflicts));
                if ((type === "user_story" && parent.length !== 1) || (type === "acceptance_criteria" && target.length !== 1))
                    return JSON.stringify(blocked("Target must resolve to one canonical existing record."));
                ...
                if (conflicts.length)
                    return JSON.stringify(blocked("Preview is incomplete or conflicted.", artifact, gaps, questions, conflicts));
                ...
                return JSON.stringify({ previewId, writeStatus: "ready_for_approval", artifactMarkdown: artifact, integrityHash, adoPreview, gaps, questions, conflicts });
            } }) } });
