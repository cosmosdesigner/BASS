...
import routeRuntime from "../../../adapters/opencode/plugins/bass-route-workflow.js"
import composeRuntime from "../../../adapters/opencode/plugins/bass-compose-response.js"
import nextRuntime from "../../../adapters/opencode/plugins/bass-recommend-next.js"
...
const args = ["--module", "node16", "--target", "es2022", "--moduleResolution", "node16", "--skipLibCheck", "--outDir", output, ...names.map(([file]) => join(root, "adapters", "opencode", "plugins", `${file}.ts`)), join(root, "support", "test-support", "d9", "opencode-plugin-shim.d.ts")]
...
for (const [file] of names) copyFileSync(join(root, "adapters", "opencode", "plugins", `${file}.js`), join(output, `${file}.js`))
