...
const safeDirectory = (root, target) => { try { return fs.existsSync(root) && fs.existsSync(target) && !fs.lstatSync(root).isSymbolicLink() && !fs.lstatSync(target).isSymbolicLink() && fs.statSync(root).isDirectory() && fs.statSync(target).isDirectory() && within(fs.realpathSync(root), fs.realpathSync(target)); } catch { return false; } };
const safeFile = (root, target) => { try { return fs.existsSync(root) && fs.existsSync(target) && !fs.lstatSync(root).isSymbolicLink() && !fs.lstatSync(target).isSymbolicLink() && fs.statSync(target).isFile() && within(fs.realpathSync(root), fs.realpathSync(target)); } catch { return false; } };
