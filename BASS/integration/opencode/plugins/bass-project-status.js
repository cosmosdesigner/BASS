...
function safeDirectory(root, target) { try { return fs.existsSync(target) && !fs.lstatSync(target).isSymbolicLink() && fs.statSync(target).isDirectory() && within(root, fs.realpathSync(target)); } catch { return false; } }
function safeFile(root, target) { try { return fs.existsSync(target) && !fs.lstatSync(target).isSymbolicLink() && fs.statSync(target).isFile() && within(root, fs.realpathSync(target)); } catch { return false; } }
