...
function safeDirectory(root, target) {
  try {
    return fs.existsSync(target) && !fs.lstatSync(target).isSymbolicLink() && fs.statSync(target).isDirectory() && within(root, fs.realpathSync(target));
  } catch { return false; }
}
...
function initProject(input) {
  ...
  if (!safeDirectory(directory, bassRoot) || !safeDirectory(bassRoot, projectsRoot)) return blocked("invalid_distribution", "BASS/projects is unavailable or outside the trusted BASS root.", "Install the complete BASS distribution before initializing a project.");
