...
const bassRoot = join(workspaceRoot, "BASS");
...
const d1Artifact = join(workspaceRoot, "BASS", "AGENTS.md");
...
for (const cwd of [workspaceRoot, bassRoot]) {
  ...
  assert.equal(rootResult.required_artifacts.find((result) => result.path === "BASS/AGENTS.md").status, "pass", `BASS/AGENTS.md must pass from ${cwd}`);
}
...
try {
  ...
  assert.equal(missingD1Result.required_artifacts.find((result) => result.path === "BASS/AGENTS.md").status, "blocked");
} finally {
  renameSync(d1ArtifactMissing, d1Artifact);
}

const harness = join(workspaceRoot, "BASS", "integration", "opencode", "plugins", "bass-context-brief.behavior-test.mjs");
...
assert.match(report, /BASS\/docs\/ba-quick-start\.md/);
...
const targetValidation = readFileSync(new URL("../docs/target-host-validation.md", import.meta.url), "utf8");
const adoFixture = readFileSync(new URL("./ado-test-fixture.md", import.meta.url), "utf8");
assert.match(targetValidation, /current `BASS\/reports\/phase-1-source-readiness\.md`.*`source_ready`/s);
assert.match(targetValidation, /exploratory only.*cannot yield a `target_ready` claim/s);
assert.match(adoFixture, /current `BASS\/reports\/phase-1-source-readiness\.md` report.*`source_ready`/s);
assert.match(adoFixture, /exploratory work.*do not evaluate it as target-readiness evidence/s);
