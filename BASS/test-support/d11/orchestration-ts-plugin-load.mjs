...
const args = ["--module", "node16", "--target", "es2022", "--moduleResolution", "node16", "--skipLibCheck", "--outDir", output, ...names.map(([file]) => join(root, "BASS", "integration", "opencode", "plugins", `${file}.ts`)), join(root, "BASS", "test-support", "d9", "opencode-plugin-shim.d.ts")]
...
for (const [file] of names) copyFileSync(join(root, "BASS", "integration", "opencode", "plugins", `${file}.js`), join(output, `${file}.js`))
