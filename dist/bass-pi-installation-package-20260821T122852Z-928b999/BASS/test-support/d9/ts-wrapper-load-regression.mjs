...
writeFileSync(join(shimRoot, "index.js"), `module.exports = require(${JSON.stringify(join(root, "BASS", "test-support", "d9", "opencode-plugin-runtime-stub.cjs"))});`, "utf8");
...
for (const name of ["bass-plan-ado-operation", "bass-execute-confirmed-ado-operation"]) {
  const original = join(root, "BASS", "integration", "opencode", "plugins", `${name}.ts`);
  const runtime = join(root, "BASS", "integration", "opencode", "plugins", `${name}.js`).replace(/\\/g, "\\\\");
  writeFileSync(join(source, `${name}.ts`), readFileSync(original, "utf8").replace(`require("./${name}.js")`, `require("${runtime}")`), "utf8");
}
const args = ["--module", "node16", "--target", "es2022", "--moduleResolution", "node16", "--skipLibCheck", "--outDir", output, join(source, "bass-plan-ado-operation.ts"), join(source, "bass-execute-confirmed-ado-operation.ts"), join(root, "BASS", "test-support", "d9", "opencode-plugin-shim.d.ts")];
