...
- Rebuilt the D8 review, improvement, and approved-persistence tools in `BASS/integration/opencode/plugins/`.
...
- `node BASS/integration/opencode/plugins/bass-review-artifact.behavior-test.mjs` failed because traversal normalized to an allowed read.
- `node BASS/integration/opencode/plugins/bass-improve-artifact.behavior-test.mjs` failed because the unresolved entry was not explicitly labeled `needs_decision`.
- `node BASS/integration/opencode/plugins/bass-persist-approved-improvement.behavior-test.mjs` failed because the immutable record omitted required original-report, unresolved/waived, and approval sections.

GREEN verification:

- `node BASS/integration/opencode/plugins/bass-review-artifact.behavior-test.mjs` passed.
- `node BASS/integration/opencode/plugins/bass-improve-artifact.behavior-test.mjs` passed.
- `node BASS/integration/opencode/plugins/bass-persist-approved-improvement.behavior-test.mjs` passed.
